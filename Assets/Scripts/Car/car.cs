...
public class car : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        accelInput = Mathf.Clamp((Input.GetAxis("Vertical") + (Input.GetAxis("R2")+1) - (Input.GetAxis("L2")+1)), -1, 1) * 50000;

        bool handbrake = false;
        if (Input.GetKey(KeyCode.Space) || Input.GetButton("Fire2"))
            handbrake = true;

        lastoffsets1 = Spring(s1, lastoffsets1, accelInput, frontStrength, FrontGrip, handbrake);
        lastoffsets2 = Spring(s2, lastoffsets2, accelInput, frontStrength, FrontGrip, handbrake);
        lastoffsets3 = Spring(s3, lastoffsets3, accelInput, backStrength, BackGrip, handbrake);
        lastoffsets4 = Spring(s4, lastoffsets4, accelInput, backStrength, BackGrip, handbrake);
    ...
    float Spring(Transform a, float b, float c, float strength, AnimationCurve tireGrip, bool handbreak)
    {
